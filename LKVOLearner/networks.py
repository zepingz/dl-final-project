...
class VggDepthEstimator(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(0, len(self.upconv_layers)):
            ...
            if i==(len(self.upconv_layers)-1):
                x = torch.cat((x, nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)(invdepth_pyramid[-1])), 1)
            elif i > 3:
#                 print('1', x.shape)
#                 print('2', self.conv_feats[-(2+i)].shape)
#                 print('3', nn.Upsample(scale_factor=2, mode='bilinear')(invdepth_pyramid[-1]).shape)
                x = torch.cat((x, self.conv_feats[-(2+i)], nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)(invdepth_pyramid[-1])), 1)
            else:
