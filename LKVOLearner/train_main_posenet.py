...
dataset = pdataset(list_file = './train.txt', data_root_path = opt.dataroot, img_size=img_size, bundle_size=3)
...
for epoch in range(max(0, opt.which_epoch), opt.epoch_num+1):
    ...
    for ii, data in enumerate(dataloader):
        ...
        if np.mod(step_num, opt.print_freq)==0:
            ...
            t = timer()
#             print('photometric_cost:', photometric_cost.detach().cpu().item())
#             print('smoothness_cost:', smoothness_cost.detach().cpu().item())
#             print('cost:', cost.detach().cpu().item())
#            visualizer.plot_current_errors(step_num, 1, opt,
#                        # OrderedDict([('photometric_cost', photometric_cost.data.cpu()[0]),
#                        OrderedDict([('photometric_cost', photometric_cost.detach().cpu().item()),
#                         ('smoothness_cost', smoothness_cost.detach().cpu().item()),
